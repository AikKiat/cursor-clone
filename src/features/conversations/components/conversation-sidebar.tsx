...
export const ConversationSidebar = ({ projectId }: ConversationSideBarProps) => {
    ...
    const handleCreateConversation = async () => {
        try {
            ...
            toast.error("[NEXTJS CLIENT SIDE] AI conversation component: Error, unable to create new conversation");
            return null;
        }
    }
    ...
    const handleSubmit = async (message: PromptInputMessage) => {
        ...
        try {
            ...
            if (!aiResponseResult.ok) {
                throw new Error(`[AI CONVERSATION RESPONSE FETCH] Failed to get response from AI service: ${aiResponseResult.statusText}`)
            };

            console.log("aiResponseResult", aiResponseResult);

            if (aiResponseResult == null) {
                throw new Error(`[AI CONVERSATION RESPONSE FETCH] Failed to get response from AI service: aiResponseResult is null`);
            }
            ...
            toast.error("[NEXT JS CLIENT SIDE] AI Response HTTP POST fetch: Error, failed to get AI response after sending frontend message");
        }
    }
